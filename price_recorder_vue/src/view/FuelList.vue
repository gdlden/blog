...
import type { FuelVehicle } from '@/api/fuel'
...
const formData = ref<FuelVehicle>({
  id: '',
  name: '',
  plateNo: '',
  brand: '',
...
function openEditModal(vehicle: FuelVehicle) {
...
async function handleSubmit() {
  ...
  try {
    if (isEditing.value) {
      await fuelStore.updateVehicle(formData.value)
    } else {
      const { id, ...rest } = formData.value
      await fuelStore.createVehicle(rest)
    }
...
async function handleDelete(id: string) {
  ...
  try {
    await fuelStore.deleteVehicle(id)
  } catch (err: any) {
...
function goToDetail(id: string) {
  router.push({ name: 'fuelDetail', params: { vehicleId: id } })
}

function changePage(page: number) {
  if (page >= 1 && page <= totalPages.value) fuelStore.fetchVehicles(page)
}
  ...
  <div class="max-w-[1100px] mx-auto px-5 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      <div class="bg-white rounded-2xl p-5 border border-[#f0f0f0]">
        <p class="text-sm text-[#86868b] mb-2">车辆数</p>
        <p class="text-[24px] font-semibold text-[#1d1d1f]">{{ vehicleCount }}</p>
      </div>
      <div class="bg-white rounded-2xl p-5 border border-[#f0f0f0]">
      ...
      <div class="bg-white rounded-2xl p-5 border border-[#f0f0f0]">
        <p class="text-sm text-[#86868b] mb-2">记录方式</p>
        <p class="text-[18px] font-semibold text-[#1d1d1f]">加满锚点</p>
      </div>
    </div>
    ...
    <div v-else-if="vehicles.length === 0" class="text-center py-24">
      <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#f5f5f7] mb-5">
        <svg class="w-8 h-8 text-[#86868b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            stroke-linejoin="round"
            stroke-width="1.5"
            d="M3 13h2l2-5h10l2 5h2m-2 0v5a1 1 0 01-1 1h-1a2 2 0 01-4 0H9a2 2 0 01-4 0H4a1 1 0 01-1-1v-5zm4 0h10"
          />
        </svg>
      </div>
      <h3 class="text-xl font-semibold text-[#1d1d1f] mb-2">暂无车辆</h3>
      <p class="text-sm text-[#86868b] mb-6">先添加车辆，再开始记录加油</p>
      <button
        @click="openCreateModal"
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      <div
        ...
        class="group bg-white rounded-2xl p-5 flex flex-col border border-[#f0f0f0] transition-all duration-300 hover:shadow-lg hover:border-[#e8e8ed] hover:-translate-y-0.5 cursor-pointer"
      >
        <div class="flex justify-between items-start mb-2">
          ...
          <span
            v-if="vehicle.plateNo"
            class="text-xs px-2 py-0.5 rounded-full border font-medium ml-2 bg-blue-50 text-blue-700 border-blue-200"
            >{{ vehicle.plateNo }}</span
          >
        </div>
        ...
        <p class="text-[22px] font-semibold text-[#1d1d1f] mb-1">
          {{ vehicle.tankCapacity || '0' }} L
        </p>
        <p class="text-sm text-[#86868b] line-clamp-2 flex-1 mb-4">
          {{ vehicle.remark || '无备注' }}
        </p>
        <div
        ...
        >
          <button
            @click.stop="openEditModal(vehicle)"
      ...
      >
        ...
        >
          ...
          <div class="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block mb-1.5 text-sm font-medium text-[#1d1d1f]">车辆名称</label
              ><input
            ...
            <div>
              <label class="block mb-1.5 text-sm font-medium text-[#1d1d1f]">车牌号</label
              ><input
            ...
            <div>
              <label class="block mb-1.5 text-sm font-medium text-[#1d1d1f]">品牌</label
              ><input
            ...
            <div>
              <label class="block mb-1.5 text-sm font-medium text-[#1d1d1f]">型号</label
              ><input
            ...
            <div>
              <label class="block mb-1.5 text-sm font-medium text-[#1d1d1f]">油箱容量 (L)</label
              ><input
                v-model="formData.tankCapacity"
                type="number"
                step="0.01"
                placeholder="请输入油箱容量"
                class="w-full px-4 py-2.5 bg-[#fafafc] border border-[#e8e8ed] rounded-xl text-[15px] text-[#1d1d1f] outline-none transition-all placeholder:text-[#c7c7cc] focus:border-[#0071e3] focus:bg-white focus:ring-2 focus:ring-[#0071e3]/10"
