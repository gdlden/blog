...
func (d *DebtDetailRepo) ListByUserId(ctx context.Context, userId string) ([]*biz.DebtDetail, error) {
	var details []DebtDetail
	err := d.data.db.WithContext(ctx).
		Table("debt_details as dd").
		Select("dd.*").
		Joins("JOIN debts d ON d.id = dd.debt_id").
		Where("d.user_id = ? AND dd.deleted_at IS NULL AND d.deleted_at IS NULL", userId).
...
func (d *DebtDetailRepo) EditDb(ctx context.Context, detail *biz.DebtDetail) {
	// TODO implement me
	panic("implement me")
}

func (d *DebtDetailRepo) DeleteDb(ctx context.Context, u uint) {
	// TODO implement me
	panic("implement me")
}
