...
**硬性约束：每次修改任何代码（包括但不限于 bug 修复、字段调整、逻辑变更）后，必须立即启动完整服务进行联调验证，确认无误后方可提交。不允许只编译或只跑单元测试就结束。**

联调验证必须覆盖：
1. 编译通过：`go build ./...`（后端）+ `pnpm build`（前端）
2. 单元测试通过：`go test ./...`（后端）+ `pnpm test:unit`（前端，如有）
3. 服务启动：`cd blog && make run` 启动后端
4. 接口验证：用 curl 或浏览器测试新增/修改的接口
5. 前端验证：`cd price_recorder_vue && pnpm dev` 启动前端，用浏览器验证页面交互和数据显示

**测试步骤：**
1. `go test ./...` 跑后端测试
2. `cd blog && make run` 启动后端
3. `cd price_recorder_vue && pnpm dev` 启动前端
4. 用 curl 或浏览器验证新增/修改的接口
5. 用浏览器验证前端页面交互和数据显示
